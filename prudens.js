...
function linearPriorities(rule1, rule2, kb, sub) { // true if rule1 is of HIGHER priority than rule2.
    return kb.indexOf(rule1) > kb.indexOf(rule2);
}

function specificityPriorities(rule1, rule2, kb, sub) { // true if rule1 is of HIGHER priority than rule2.
...
function updateGraph(inferredHead, newRule, graph, facts, priorityFunction, deletedRules, sub, constraints, kb, dilemmas) { //TODO You may need to store the substitution alongside each rule, in case one needs to count how many time a rule has been triggered or so.
    let inferred = false;
    ...
    for (const oppositeHead of conflicts) {
        ...
        if (deepIncludes(oppositeHead, facts, true)) {
            ...
            for (const rule of graph[literalToString(oppositeHead)]) {
				isPrior = priorityFunction(newRule, rule, kb, sub);
    ...
    do {
        ...
        for (let i=0; i<kb.length; i++) {
            ...
            for (let i=0; i<subs.length; i++) {
                ...
                const updatedGraph = updateGraph(inferredHead, rule, graph, facts, priorityFunction, deletedRules, sub, kbObject["constraints"], kb, dilemmas);
